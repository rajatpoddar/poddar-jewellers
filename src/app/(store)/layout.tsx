...
import { getNavCategories } from '@/lib/categories.server';
...
export default async function StoreLayout({ children }: { children: ReactNode }) {
  ...
  const categories = await getNavCategories(shop.id);
  ...
  return (
    <div className="min-h-screen bg-ground text-ink font-body flex flex-col antialiased">
      ...
      <Header shop={shop} customer={customer ? { name: customer.name } : null} categories={categories} />
