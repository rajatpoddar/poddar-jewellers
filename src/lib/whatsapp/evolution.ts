...
/**
 * Sends a text message via Evolution API HTTP POST.
 */
export async function sendEvolutionApiMessage(
  ...
  if (!shop.evolutionApiUrl || !shop.evolutionInstance) {
    throw new Error('Missing Evolution API configuration (evolutionApiUrl or evolutionInstance)');
  }

  const formattedPhone = formatEvolutionPhone(phone);
  const payload = buildEvolutionPayload(formattedPhone, text);
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `Evolution API HTTP error ${response.status}${errorBody ? `: ${errorBody}` : ''}`
    );
  }
